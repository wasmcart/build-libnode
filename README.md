...
### The snapshot stub (required on Windows, harmless everywhere)

`node::SnapshotBuilder::GetEmbeddedSnapshotData()` is generated by mksnapshot
into the node BINARY, never into the library. The linux and macOS archives
fold Node's null-snapshot stub object into `libnode.a`, but no Windows .lib
carries it, so an embedder must supply the stub (wasmcart-native and
jsgame-libretro both do):
...
Windows also needs `/MT` (the library is built against the static CRT; `/MD`
fails with LNK2038) and `/std:c++20 /Zc:__cplusplus` for V8's headers.
`npx libnode-config --libs` lists the system libraries for each platform.
